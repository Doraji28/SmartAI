...
-- Spawn flashy explosive particle burst for PERFECT hits
function fx_manager.spawn_perfect_burst(x, y, color)
    color = color or {1, 0.9, 0.2, 1}
    for i = 1, 35 do -- more particles
        local angle = math.random() * math.pi * 2
        local speed = math.random(150, 450) -- faster explosion velocity
        local p = {
            x = x,
            y = y,
            dx = math.cos(angle) * speed,
            dy = math.sin(angle) * speed, -- radial burst direction
            gravity = 120, -- lower gravity so they stay floating outward
            color = {color[1], color[2], color[3], 1.0},
            size = math.random(3, 6), -- larger particles
            age = 0,
            max_age = math.random(4, 9) * 0.1 -- slightly longer lifetime
        }
        table.insert(fx_manager.particles, p)
    end
end
